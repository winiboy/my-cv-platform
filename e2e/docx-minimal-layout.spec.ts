...
/**
 * US-006: the minimal DOCX takes its section order, its section visibility and
 ...
 *
 * US-006 forbids Minimal's output changing, so there is no defect for this
 * spec to catch: it is green at the commit that introduces it. Each assertion
 * group was therefore shown to DISCRIMINATE by mutating `docx-minimal.ts` one
...
assertLocalSupabase(SUPABASE_URL, 'E2E minimal DOCX layout spec')
 ...
 *
 * These are the same three values the Classic spec seeds, and that is a choice
 * rather than an oversight: the shape is picked for being maximally
 * discriminating, and varying it here for the sake of variety would trade
 * discrimination for novelty. What the two specs assert about the resulting
 * document is not the same, because the two documents are not.
 */
 ...
 * nowhere else in the document — each verified to occur exactly once in the
 * generated artifact.
 *
 ...
 * `w:sz` as an ATTRIBUTE (`<w:top w:val="single" w:sz="4" …>`), and Minimal
 * draws several of them, so a looser pattern would match a border width and
 * make the negative assertion below fail for a reason that has nothing to do
 * with typography. `w:szCs` — the complex-script twin `docx` emits alongside —
 * is matched too, and carries the same value.
 */
...
test('the minimal DOCX takes section order, visibility and type scale from the account', async ({
  page,
  authedUser,
}) => {
  const resume = await seedFixtureResume(authedUser.id, 'minimal')
  ...
  // --- Section order ---
  //
  // Minimal is single-column, so document order IS section order — there is no
  // table row splitting the body into columns as there is in Modern. The one
  // table Minimal emits sits INSIDE the combined languages + certifications
...
test('the seeded layout really is non-default, in every property this file asserts', () => {
   ...
   *
   * The order comparison is made AFTER the Minimal mapping, because that is the
   * vocabulary the document is written in: two different editor orders can map
   * to the same Minimal order, and it is the mapped pair that has to differ.
   */
  const seeded = mapEditorOrderToMinimal(ACCOUNT_LAYOUT.mainContentOrder)
  const defaults = mapEditorOrderToMinimal(DEFAULT_RESUME_LAYOUT.mainContentOrder)
